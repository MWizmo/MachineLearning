...
def h(x, theta):
    return theta[0] - x[0] * theta[1] + x[1] * theta[2] - theta[3]
...
def gradient(x, y, theta, alpha, iters):
    ...
    for i in range(0, iters):
        ...
        theta[3] = theta[3] - (alpha / len(x)) * _sum0
    return theta, costs
...
theta = theta_opt = np.zeros(4)
print("Целевая функция в начале обучения:", J(x, y, theta))
J_min = J(x, y, theta)
best_alpha = 0
start = 0.001
for alpha in range(1, 11):
    theta1, costs = gradient(x, y, theta, start, 1000)
...
_x = np.linspace(-1, 2, 100)
plt.scatter(x_test, y_test)
_y = np.array([np.exp(theta_opt[0] - theta_opt[1]*_x[i]**2+theta_opt[2]*_x[i]-theta_opt[3]) for i in range(0,100)])
plt.plot(_x, _y, c='r')
